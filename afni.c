...
static char *correlation[4] = { "Pearson Correlation" , "Mutual Information" , "Binary" , "Kendall's Coefficient of Concordance" } ;
static char *represent[4] = { "Single Voxel" , "Average of ROI" , "PCA in ROI" , "Each Voxel in ROI" } ;
static char *chooseROI[6] = { "Single Voxel" , "Cube" , "Cuboid" , "Sphere" , "Ellipse" , "Atlas Region" } ;

static char *size_vs_voxels[2] = { "Size in mm" , "Number of Voxels"};
...
void AFNI_fctoolbar_CB( Widget w, XtPointer client_data , int nval, void **val)
{
    Three_D_View *im3d = (Three_D_View *)client_data ;

ENTRY("AFNI_fctoolbar_CB") ;
    
    printf("%s \n", im3d->window_title);

    if (im3d->type == AFNI_3DDATA_VIEW){

        // Create Interfaces for sphere, cube , cuboid etc. inputs 

        // Save the value of val in oldval

        BBOX_set_wtype("Parameters");
        ...
        if(strcmp(ROIvalue,chooseROI[0]) == 0){
                AFNI_fctool_CB(im3d->vwid->imag->fctoolbar, (XtPointer)im3d ,nval,val);
        }
        else if(strcmp(ROIvalue,chooseROI[1]) == 0){
            MCW_choose_stuff(im3d->vwid->imag->fctoolbar,
                            "Enter Parameters for Cube Size",
                            AFNI_fctool_CB , (XtPointer) im3d,
                            MSTUF_STRLIST , "Size (Half of cube's edge)" , 2 , 0 , size_vs_voxels ,
                            MSTUF_STRING , "Enter the Value",    
                            MSTUF_END ) ;
        }
        else if(strcmp(ROIvalue,chooseROI[2]) == 0){
            MCW_choose_stuff(im3d->vwid->imag->fctoolbar,
                            "Enter Parameters for Cuboid Size",
                            AFNI_fctool_CB , (XtPointer) im3d,
                            MSTUF_STRLIST , "Edge Length (X direction)" , 2 , 0 , size_vs_voxels ,
                            MSTUF_STRING , "Enter the Value",
                            MSTUF_STRLIST , "Edge Length (Y direction)" , 2 , 0 , size_vs_voxels ,
                            MSTUF_STRING , "Enter the Value",
                            MSTUF_STRLIST , "Edge Length (Z direction)" , 2 , 0 , size_vs_voxels ,
                            MSTUF_STRING , "Enter the Value",    
                            MSTUF_END ) ;
        }
        else if(strcmp(ROIvalue,chooseROI[3]) == 0){
            MCW_choose_stuff(im3d->vwid->imag->fctoolbar,
                            "Enter Parameters for Sphere Size",
                            AFNI_fctool_CB , (XtPointer) im3d,
                            MSTUF_STRLIST , "Radius of Sphere" , 2 , 0 , size_vs_voxels ,
                            MSTUF_STRING , "Enter the Value",    
                            MSTUF_END ) ;
        }
        else if(strcmp(ROIvalue,chooseROI[4]) == 0){
            MCW_choose_stuff(im3d->vwid->imag->fctoolbar,
                            "Enter Parameters for Ellipse Size",
                            AFNI_fctool_CB , (XtPointer) im3d,
                            MSTUF_STRLIST , "Radius (X direction)" , 2 , 0 , size_vs_voxels ,
                            MSTUF_STRING , "Enter the Value",
                            MSTUF_STRLIST , "Radius (Y direction)" , 2 , 0 , size_vs_voxels ,
                            MSTUF_STRING , "Enter the Value",
                            MSTUF_STRLIST , "Radius (Z direction)" , 2 , 0 , size_vs_voxels ,
                            MSTUF_STRING , "Enter the Value",    
                            MSTUF_END ) ;
        }
        else{
            ...
            if(at_labels){
                ...
                MCW_choose_strlist(im3d->vwid->imag->fctoolbar,
                           title_str ,
...
void AFNI_fctool_CB( Widget w, XtPointer client_data , int nval, void **val)
{
    Three_D_View *im3d = (Three_D_View *)client_data ;

ENTRY("AFNI_fctool_CB") ;
...
void AFNI_compfc_CB(Widget w, XtPointer client_data, XtPointer call_data)
{
    Three_D_View *im3d = (Three_D_View *)client_data;

ENTRY("AFNI_compfc_CB");

    //printf("%s \n", (char*)oldval[0]);

    FILE * fp = fopen("fconn.txt", "w+");

	THD_3dim_dataset *dset = im3d->anat_now;

	//THD_3dim_dataset *dset1 = im3d->fim_dset[];

    int ns = dset->dblk->nvals ; // num of series
    int nv = DSET_NVOX(dset);  // num of voxels

    float *corr;
    corr = (float *)calloc(nv,sizeof(float) );
    short * ts; 
    int j,k;

	printf("Number of Voxels = %d",nv);
	printf("Number of Series = %d",ns);
	
	//printf("current location is %s \n", im3d->icalc_setup->prefix);

    int vx = (int)((im3d->vinfo->xi-DSET_XORG(dset))/DSET_DX(dset));
    int vy = (int)((im3d->vinfo->yj-DSET_YORG(dset))/DSET_DY(dset));
    int vz = (int)((im3d->vinfo->zk-DSET_ZORG(dset))/DSET_DZ(dset));
    int index = DSET_ixyz_to_index(dset,vx,vy,vz);
    ...
    for(k=0;k<nv;k++){
      float exy=0.0,exx=0.0,eyy=0.0, ex=0.0, ey=0.0;
      for(j=0;j<ns;j++){
       // if(typ == MRI_float){
          ts = (short *) DSET_ARRAY(dset,j) ;
       // }//else{
       //   ts = (float *)malloc(sizeof(float)*nv) ;
       //   xar = DSET_ARRAY(dset,j);
         // EDIT_coerce_type(nv,typ,xar,MRI_float,ts) ;
      //  } 
        exy += (float)ts[index]*(float)ts[k];
       // printf("%f %u\n", (float)ts[index], ts[index] );
        ex += (float)ts[index];
        ey += (float)ts[k];
        exx += (float)ts[index]*(float)ts[index];
        eyy += (float)ts[k]*(float)ts[k];
      }

      //printf("%d %f %f %f %f %f %f %f\n",ns, exy, ex, ey, exx, eyy, sqrt(ns*exx-ex*ex), sqrt(ns*eyy-ey*ey));
		float den = (sqrt(ns*exx-ex*ex)*sqrt(ns*eyy-ey*ey));
		float num = ns*exy-ex*ey;
      	
		if(den!=0){corr[k] = num/den;}
		else if(num==0){corr[k]=0;}
		else{corr[k]=1;}
      //printf("%f\n",corr[k] );
    }

    for(j=0;j<nv;j++){
      fprintf(fp,"%f %s", corr[j], "\n");
    }
   ...
   // qset = EDIT_full_copy(im3d->fim_now,"copy");
	qset = EDIT_empty_copy(dset) ;
	
	//THD_copy_datablock_auxdata( qset->dblk , dset->dblk );
    //INIT_STAT_AUX( dset , MAX_STAT_AUX , qset->stat_aux ) ;
	
	//printf("MAX_STAT_AUX is %d\n", MAX_STAT_AUX);

    //float fbuf[MAX_STAT_AUX]; 
	//int ii;
	//for( ii=0 ; ii < MAX_STAT_AUX ; ii++ ) fbuf[ii] = 0.0 ;

	//THD_erase_one_atr( qset->dblk , char *name );

	tross_Copy_History(dset,qset) ;
	EDIT_dset_items( qset ,
                        ADN_prefix    , "Connectivity" ,
                        ADN_nvals     , 1 ,
                        ADN_ntt       , 1 ,
	...
	EDIT_substitute_brick( qset , 0 , MRI_float , (float *)corr ) ;

	printf("FUNC TYPE IS %d \n", qset->func_type);
	
	float * mb = (float *) DSET_ARRAY(qset,0);
    
	THD_3dim_dataset *dseTT = TT_retrieve_atlas_dset(Current_Atlas_Default_Name(),0);
	
	if(dseTT==NULL){printf("You don't have %s\n",Current_Atlas_Default_Name());}
	else{
		float * mb1 = (float * ) DSET_ARRAY(dseTT,0);
		for(j=0;j<nv;j++){
		  printf("%f %s", (float)mb1[j], "\n");
		}
		printf("I have Current Atlas \n");
		THD_write_3dim_dataset(NULL, "My_Atlas", dseTT, True);		
	}
	
	int vv = qset->view_type ;
	printf("View Type is %d \n",vv);
    int nds = im3d->ss_now->num_dsset;
//if(DEBUG) ININFO_message("adding to session: nds=%d",nds) ;
    SET_SESSION_DSET(qset, im3d->ss_now, nds, vv);
    im3d->ss_now->num_dsset++;
    //AFNI_force_adoption( im3d->ss_now , False );
    //AFNI_make_descendants( GLOBAL_library.sslist );
    INFO_message("created new dataset %s","Connectivity");

	INFO_message("Writing %s","Connectivity");
	//THD_load_statistics(qset) ;	
	THD_write_3dim_dataset( NULL, "Connectivity" , qset , True );
	
	if(ISVALID_3DIM_DATASET(qset)){printf("Yes baby the dataset is a valid one :) \n");}
	
	int nx = qset->daxes->nxx;
	int ny = qset->daxes->nyy;
	int nz = qset->daxes->nzz;
//	int nt = qset->taxis->ntt;
	int nxyz = nx * ny * nz;
	
	printf("%d %d %d \n", nx,ny,nz);
	
	//qset->dblk->diskptr->allow_directwrite = 1 ;
...
//if(DEBUG) ININFO_message("turn overlay on");
	...
	mri_add_name("Finally",qim);
 //   void * handle;
//    handle = PLUTO_popup_image(NULL, qim); 
	
	/*
    FD_brick * fdbrick = THD_3dim_dataset_to_brick( qset , 3, 2, 1);
    printf("%f \n" , fdbrick->del1);
    MRI_IMAGE * over = AFNI_func_overlay( 0 , fdbrick );
    printf("I am here \n");
    mri_write_jpg("overlay.jpg",over);
    */
   	
	/*
	int nx = im3d->fim_now->daxes->nxx; int ny = im3d->fim_now->daxes->nyy ; int nz = im3d->fim_now->daxes->nzz;
    MRI_IMAGE *qim  = mri_new_vol(nx, ny, nz , MRI_float ) ;
    float *qar  = MRI_FLOAT_PTR(qim) ;
    for( k=0; k<nv; k++) qar[k] = corr[k] ;
    mri_write_jpg("overlay.jpg",qim);
	*/
}
...
ENTRY("AFNI_imag_pop_CB") ;
   ...
   /* 4 May 2015 : Functional Connectivity options */

   else if( w == im3d->vwid->imag->seedfc &&
            im3d->type == AFNI_3DDATA_VIEW             ){

      if( ISQ_REALZ(seq) ){
         AFNI_compfc_CB(w,(XtPointer)im3d,call_data);
      }
   }

   else if( w == im3d->vwid->imag->fctoolbar &&
            im3d->type == AFNI_3DDATA_VIEW             ){

      if( ISQ_REALZ(seq) ){

        MCW_choose_stuff( seq->wbar , 
                        "Choose Options for Functional Connectivity" ,
                        AFNI_fctoolbar_CB , (XtPointer) im3d ,
                        MSTUF_STRLIST , "Select ROI " , 6 , 0 , chooseROI ,
                        MSTUF_STRLIST , "Correlation Method " , 4 , 0 , correlation ,
                        MSTUF_STRLIST , "Representative Time Series " , 4 , 0, represent,
                        MSTUF_INT , "Number of Principal Components for PCA " , 1 , 5 , 1, 
                        MSTUF_STRLIST , "Regress out Motion Correction ", 2 , 0 , yesno,
                        MSTUF_STRLIST , "Regress out Experimental Conditions ", 2 , 0 , yesno,
                        MSTUF_STRLIST , "Regress out Average GM signal ", 2 , 0 , yesno,
                        MSTUF_STRLIST , "Regress out WM , Ventricle signals ", 2 , 0 , yesno,
                        MSTUF_STRLIST , "Regress out Cardiac and Respiratory signals ", 2 , 0 , yesno,    
                        MSTUF_END ) ;
      }
   }
